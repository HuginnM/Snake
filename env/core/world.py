...
class World(object):
    def __init__(self, size, custom, start_position, start_direction_index, food_position):
        ...
        self.world[self.size[0]-1] = self.WALL
        self.world[:, 0] = self.WALL
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = ((random.randint(SNAKE_SIZE, self.size[0] - SNAKE_SIZE),
                               random.randint(SNAKE_SIZE, self.size[0] - SNAKE_SIZE)))
    ...
    def init_food(self):
        ...
        available_food_positions = set(zip(*np.where(self.world == 0))) - set(snake.blocks)
        print(snake.blocks)
        if not self.custom:
